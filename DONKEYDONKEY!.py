...
patrones = [
    r'(/\d{3,})(.*?)(\d{3})',           # Patrón #1: "/" seguido de al menos tres dígitos, cualquier cosa, luego tres dígitos.
    r'(/\d{3,}\s*)([A-Za-z]+)(\d{3})',   # Patrón #2: "/" seguido de al menos tres dígitos, espacios, letras, y luego tres dígitos.
    r'(/\d{3,}\s*)(\S+)(\d{3})',          # Patrón #3: "/" seguido de al menos tres dígitos, espacios, caracteres no vacíos, y tres dígitos.
    r'(/\d{3,})((\s*)\S+)' 
]         


# Compilar patrones para mejorar rendimiento y control
patrones_compilados = [re.compile(p) for p in patrones]
...
def extract_text(image):
    ...
    extracted_text = extracted_text[:115]
    return extracted_text

def buscaPatron(text):
    for c, p in enumerate(patrones, start=1):
        coincidencia = re.search(p, text)
    ...
    for item in os.listdir(folder):
        ...
        try:
         os.rename(image_path, os.path.join(folder, nuevo_nombre))      
            ...
            print("Burro no pudo renombrar '"+image_path+"', seguro hay un archivo repetido")
